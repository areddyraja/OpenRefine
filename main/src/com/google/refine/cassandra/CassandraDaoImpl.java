...
public class CassandraDaoImpl {
    ...
    public void saveMetadata(ProjectMetadata metaData, long projectId)
        ...
        try {
            ...
            PreparedStatement pstmt= session.prepare("insert into demodb.metadata_tbl(project_id,name,created,modified,custommetadata,password,encoding,encodingconfidence,preferences) values(?,?,'now','now',?,?,?,?,?)");
            
            BoundStatement boundStatement = new BoundStatement(pstmt);
            session.execute(boundStatement.bind(projectId,metaData.getName(),CassandraUtil.getCustomMetaData(metaData) ,metaData.getPassword(),metaData.getEncoding(),metaData.getEncodingConfidence(),CassandraUtil.getPreferences(metaData)));
    ...
    public JSONObject loadMetaData(long projectId) throws JSONException
    {
        ...
        try {
            ...
            PreparedStatement pstmt= session.prepare("select * from metadata_keyspace.metadata_tbl where project_id=?");
...
//                    logger.info("pre"+"::"+row.getString("preferences"));
    ...
    public void saveProject(Project project)
        ...
        try {
            ...
            PreparedStatement pstmt= session.prepare("insert into metadata_keyspace.metadata_tbl(project_id,name,created,modified,custommetadata,password,encoding,encoding_confidence,preferences) values(?,,?,'now','now',?,?,?,?,?)");
